...
module.exports.config = {
	...
	description: "Đọc truyện trên Messenger!",
	commandCategory: "Truyện",
	usages: "nettruyen [name]",
	cooldowns: 0
	...
	var t = this,
		...
		n = e.event,
		r = e.args;
	if (!r[0]) return a.sendMessage("Nhập từ khoá !", n.threadID);
	for (var i = await got("https://www.nettruyenus.com/tim-truyen?keyword=" + encodeURIComponent(r.join(" "))), o = cheerio.load(i.body), s = o("#ctl00_divCenter").find(".Module-170 > div > div > div > div > figure > div > a"), d = [], c = 0; c < s.length; c++) {
	...
	var p = d.length,
		g = 1;
	(g = parseInt(r[1]) || 1) < -1 && (g = 1), Math.ceil(p / 10);
	for (var m = "", f = [], y = 10 * (g - 1); y < 10 * (g - 1) + 10 && !(y >= p); y++) {
		var v = d[y].title,
	...
	var t = this,
		...
		n = e.api,
		r = e.event;
	if (a.author != r.senderID) return n.sendMessage("Sai số!", r.threadID);
	switch (a.type) {
		case "info":
			...
			s.reverse();
			var u = s;
			return n.sendMessage("???/" + s.length + " chap", r.threadID, (function(e, a) {
				global.client.handleReply.push({
					name: t.config.name,
					messageID: a.messageID,
					author: r.senderID,
		...
		case "read":
			...
			var w, x = async function(e) {
				await axios({
					...
					responseType: "stream",
					headers: {
						Referer: "http://www.nettruyenus.com/",
